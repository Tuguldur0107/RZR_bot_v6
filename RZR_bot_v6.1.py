...
ARROW_RE   = re.compile(r"→\s*(-?\d+)")  # ж: “→ 17”

# ---------- interaction metadata-г аюулгүй авах ----------
...
# ---------- content + EMBED (title/desc/fields/footer)-ийг нэгтгэх ----------
def _full_text(m: discord.Message) -> str:
    parts = [m.content or ""]
    for e in (m.embeds or []):
        if e.title: parts.append(str(e.title))
        if e.description: parts.append(str(e.description))
...
# ---------- channel-н түүхийг (cutoff-с хойш) ачаалалтайгаар унших ----------
...
# ---------- Мөр бүрээс mention-ууд + тоонуудыг холбоод унших туслахууд ----------
def _parse_donations(text: str, bot_id: int | None = None):
    """
    Нэг мессежээс олон донорыг ч дэмжинэ:
    mention-тэй мөр бүр дээрх хамгийн ойр мөнгөн дүнг тухайн mention-д холбож буцаана.
    Буцаалт: [(uid:int, amount:int)]
    """
    results = []
    for raw_line in text.splitlines():
        ...
        # мөрөнд олон дүн байвал эхнийхийг авна (ихэнхдээ 1)
        amt = None
        if amts:
            amt = int(amts[0].group(1).replace(",", "").replace(".", ""))
        for m in ments:
            uid = int(m.group("uid"))
...
def _parse_scores_block(text: str, default_tier="4-1"):
    """
    Нэг мессежээс олон тоглогчийн state-г уншина.
...
async def leaderboard(interaction: discord.Interaction, limit: int = 10):
    ...
    def _calc_winrate(w: int, l: int) -> float:
        t = w + l
        return (w / t * 100.0) if t > 0 else 0.0

    def _tier_style_safe(tier: str) -> tuple[discord.Color, str]:
        ts = globals().get("tier_style")
        if callable(ts):
            try: return ts(tier)
            except Exception: pass
        return discord.Color(0x5865F2), "🔹"

    timeout = globals().get("GET_SCORE_TIMEOUT", 10)
    limit = max(1, min(limit, 20))
    RANK_EMOJI = {1: "🥇", 2: "🥈", 3: "🥉"}
    _num_fn = globals().get("_num")  # байвал 1 000 форматлана

    # Зураас (inline code хэлбэрээр нимгэн, тогтвортой урттай)
    SEPARATOR = "`" + ("─" * 38) + "`"
    ...
    # ---- оноо авах ----
    try:
        all_scores = await asyncio.wait_for(get_all_scores(), timeout=timeout)
    except asyncio.TimeoutError:
    ...
    # ---- эрэмбэлэлт ----
    try:
        sorted_data = sorted(all_scores.items(), key=lambda kv: tier_score(kv[1]), reverse=True)
        top = sorted_data[:limit]
        uid_list = [_to_int(uid) for uid, _ in top]
    except Exception as e:
        return await interaction.followup.send(f"⚠️ Эрэмбэлэх үед алдаа: {e}")

    # ---- статистик ----
    try:
        stat_rows = await asyncio.wait_for(get_player_stats(uid_list), timeout=timeout)
    except asyncio.TimeoutError:
        stat_rows = []
    except Exception as e:
    ...
    for r in (stat_rows or []):
        try:
            uid = _to_int(r.get("uid"))
            stat_map[uid] = {
                "wins": _to_int(r.get("wins"), 0),
                "losses": _to_int(r.get("losses"), 0),
            }
        except Exception:
            continue

    # ---- embed ----
    top_tier = _short((top[0][1].get("tier") if top else "E") or "E", 10)
    colour, _ = _tier_style_safe(top_tier)
    emb = discord.Embed(
        title=f"🏅 Leaderboard — Top {len(top)}",
        description="_Tier • +Score • 🏆/💀 • Winrate_",
        colour=colour,
        timestamp=datetime.now(timezone.utc),
    )
    ...
    errors: list[str] = []
    add_dividers = len(top) <= 12  # 25 field лимит давахгүй байх

    for i, (uid_raw, data) in enumerate(top, 1):
        try:
            uid = _to_int(uid_raw)
            ...
            tier = _short(data.get("tier", "E"), 10).upper()
            score = _to_int(data.get("score"), 0)

            # tier emoji/өнгө
            _, tier_emoji = _tier_style_safe(tier)

            st = stat_map.get(uid, {"wins": 0, "losses": 0})
            wins = _to_int(st.get("wins"), 0)
            losses = _to_int(st.get("losses"), 0)
            winrate = _calc_winrate(wins, losses)

            w_str = _num_fn(wins) if callable(_num_fn) else str(wins)
            l_str = _num_fn(losses) if callable(_num_fn) else str(losses)
            ...
            # ——— зураас
            if add_dividers and i < len(top):
                emb.add_field(name="\u200B", value=SEPARATOR, inline=False)

        except Exception as e:
            errors.append(f"#{i} uid={uid_raw}: {type(e).__name__} — {e}")
            continue
    ...
    if errors:
        emb.set_footer(text=f"Эвдэрсэн мөр: {len(errors)} • First: {errors[0]}")
    else:
    ...
    try:
        ...
        lines = [f"🏅 Leaderboard — Top {len(top)}", ""]
        for i, (uid_raw, data) in enumerate(top, 1):
            try:
                uid = _to_int(uid_raw)
                member = interaction.guild.get_member(uid) if interaction.guild else None
                username = _short(data.get("username") or (member.display_name if member else f"User {uid}"), 40)
                tier = _short(data.get("tier", "E"), 10).upper()
                score = _to_int(data.get("score"), 0)
                st = stat_map.get(uid, {"wins": 0, "losses": 0})
                wins = _to_int(st.get("wins"), 0); losses = _to_int(st.get("losses"), 0)
                winrate = _calc_winrate(wins, losses)
                lines.append(f"{RANK_EMOJI.get(i, f'#{i}')} {tier} | {score:+} — {username}  🏆{wins} / 💀{losses} — {winrate:.1f}%")
                if add_dividers and i < len(top): lines.append("—" * 40)
            except Exception:
                continue
        await interaction.followup.send("\n".join(lines)[:2000])
...
# =============================================================================
# /restore_donators — embed/текстээс донорыг мөр тус бүрээр уншаад UID-аар нэгтгэнэ
# =============================================================================
@bot.tree.command(name="restore_donators", description="Тодорхой channel-оос сүүлийн N хоногийн донатыг сэргээх")
@app_commands.describe(channel="Ямар channel-оос сэргээх вэ?", days="Хайх хоногийн тоо (1-180)")
...
async def restore_donators(
    ...
    for m, info in pairs:
        # зөвхөн add_donator команд (interaction.name) — гэхдээ зарим хуучин лог дээр нэр нь дутсан байж магадгүй
        if info["cmd"] and info["cmd"] != "add_donator":
            continue

        text = _full_text(m)
        found = _parse_donations(text, bot_id=bot_id)
        if not found:
            # fallback: бүх message дээр mention + amount хоёул байна уу гэдгийг дахин шалгана
            found = _parse_donations(m.content or "", bot_id=bot_id)

        for uid, amt in found:
    ...
    for uid, v in totals.items():
        try:
            # Танай талд байгаа upsert_donator(uid, amount) нь нийт дүнг нэмж хадгалдаг гэж үзэж байна
            await upsert_donator(uid, v["amount"])
            restored += 1
        except Exception as e:
            print(f"❌ upsert_donator uid={uid} алдаа: {e}")

    await interaction.followup.send(
        f"💖 {channel.mention} дотор **{restored} донор** сэргээв. (сүүлийн {days} өдөр)\n"
        f"ℹ️ Мессеж бүрийн mention+дүнг холбож UID-аар **нийлбэрлэж** бичсэн."
    )

# =============================================================================
# /restore_scores — add_score / set_match_result(_fountain) / set_tier-ээс state-г сэргээх
# =============================================================================
@bot.tree.command(name="restore_scores", description="Тодорхой channel-оос сүүлийн N хоногийн оноо/түйрийг сэргээх")
@app_commands.describe(channel="Ямар channel-оос сэргээх вэ?", days="Хайх хоногийн тоо (1-180)")
@app_commands.checks.has_permissions(administrator=True)
async def restore_scores(
    ...
    pairs = await _scan_channel(channel, cutoff)
    # uid бүрийн хамгийн сүүлчийн state
    ...
    for m, info in pairs:
        ...
        text = _full_text(m)
        block = _parse_scores_block(text, default_tier="4-1")
        if not block:
            # add_score / set_tier хариу нь ганц мөрөөр ирдэг тохиолдолд content-оос дахин оролдоно
            block = _parse_scores_block(m.content or "", default_tier="4-1")
    ...
    for uid, st in latest.items():
        ...
        try:
            username = await _username_for(channel.guild, uid)
            await upsert_score(uid, score, tier, username)
            restored += 1
        except Exception as e:
    ...
    await interaction.followup.send(
        ...
        f"ℹ️ Embed болон текстийн мөр бүрээс <@uid> гарж ирсэн **шугамын шинэ оноо/түйр**-ийг авч хамгийн сүүлийн төлөвт дөхүүлсэн."
    )
